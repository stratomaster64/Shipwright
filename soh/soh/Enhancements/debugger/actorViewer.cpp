...
const std::map<u16, std::string> actorDescriptions{
    { ACTOR_PLAYER, "Link" },
...
void DrawActorViewer(bool& open) {
    ...
    if (ImGui::TreeNode("Selected Actor")) {
        ...
        const auto& description = actorDescriptions.find(display.id);
        std::string descriptionString = (description != actorDescriptions.end() ? description->second : "");
        DrawGroupWithBorder([&]() {
            ImGui::Text("Name: %s", dispOverlay != nullptr ? dispOverlay->name : "???");
            ImGui::Text("Description: %s", descriptionString.c_str());
